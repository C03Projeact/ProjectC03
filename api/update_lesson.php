...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['lesson_id']) || !isset($_POST['lesson_name'])) {
    ...
    try {
        ...
        $query = "SELECT content_file FROM lesson WHERE lesson_id = :lesson_id";
        ...
        if ($updateContentFile) {
            ...
            $upload_dir = 'lesson_storage/';
            $upload_path = $upload_dir . basename($file_name);

            // Move the uploaded file to the lesson_storage directory
        ...
        if ($updateContentFile) {
            $query = "UPDATE lesson SET lesson_name = :lesson_name, content_file = :content_file, upload_time = :upload_time, status = :status WHERE lesson_id = :lesson_id";
        } else {
            $query = "UPDATE lesson SET lesson_name = :lesson_name, upload_time = :upload_time, status = :status WHERE lesson_id = :lesson_id";
        }
