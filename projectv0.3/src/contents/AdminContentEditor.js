...
const AdminContentEditor = () => {
  ...
  const fetchLessons = async () => {
    try {
      ...
      console.log("Fetched lessons data:", response.data); // ดูข้อมูลที่ได้รับ

      const data = response.data;

      // ตรวจสอบว่า data เป็น Array หรือไม่
      if (Array.isArray(data)) {
        setLessons(data); // ตั้งค่าบทเรียน
      } else {
        console.error("Error: Lessons data is not an array", data);
        setLessons([]); // ตั้งเป็นอาร์เรย์ว่าง
      }
    } catch (error) {
      console.error("Error fetching lessons:", error);
      setLessons([]); // ตั้งเป็นอาร์เรย์ว่างในกรณีเกิดข้อผิดพลาด
    }
  };


  const handleDeleteLesson = async (lesson_name) => {
    if (window.confirm("คุณแน่ใจหรือว่าต้องการลบบทเรียนนี้?")) {
      try {
        const response = await axios.delete(
          apiURL + `/insert_lessons.php?lesson_name=${lesson_name}`
        );
        ...
        fetchLessons(); // โหลดข้อมูลใหม่หลังจากลบเสร็จ
  ...
  return (
    <div className="container">
      ...
      <button onClick={() => navigate("/add-lessons")}>เพิ่มบทเรียน</button>
      <br></br>
      <br></br>
      <table
        ...
        <tbody>
          {lessons.map((lesson, index) => (
            <tr key={lesson.lesson_id}>
              ...
              <td style={styles.tableCell}>{lesson.lesson_name}</td>
              <td style={styles.tableCell}>
                <a
                  href={apiURL + "/lesson_storage/" + lesson.content_file}
                  target="_blank"
              ...
              <td>
                <Link
                  to={`/edit-lessons/${lesson.lesson_id}`}
              ...
              <td style={styles.tableCell}>
                <button
                  onClick={() => handleDeleteLesson(lesson.lesson_name)}
